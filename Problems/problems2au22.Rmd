...
$$P(D|T) = \frac{P(D \cap T)}{P(T)}.$$
  ...
  Hint for calculating the denominator: the event, $T$, that the test is positive is the union of two mutually exclusive events: $T = ( T \cap D ) \cup (T \cap D^{c})$
   ...
   d. The probability you calculated in part c. is an inverse conditional probability and is referred to as the \emph{sensitivity} of the screening test. Comment on why it makes sense for this number to be so small in this example. 

   e. Now calculate the inverse conditional probability $P(D^{c}|T^{c})$. (This is called the *specificity* of the screening test. )
